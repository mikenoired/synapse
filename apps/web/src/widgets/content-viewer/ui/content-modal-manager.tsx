...
export function ContentModalManager({
	...
	const imageGallery = useMemo(() => {
		...
		return allItems
			...
			.flatMap((i) => {
				...
				if (media?.url)
					return [
						{ url: media.url, parentId: i.id, media_type: media.type, thumbnail_url: media.thumbnailUrl },
					];
				return [];
			});
	}, [allItems, item]);
	...
	if (item.type === "audio") {
		return <AudioViewerModal open={open} onOpenChange={onOpenChange} item={item} />;
	}
