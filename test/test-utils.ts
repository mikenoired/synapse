...
export class ThumbnailTestUtils {
	...
	static async generateThumbnail(
		...
		const { width = 20, height = 0, quality = 40 } = options;
	...
	static async generateVideoThumbnail(
		videoBuffer: Buffer,
		mimeType: string,
		options: { width?: number; height?: number; quality?: number; timestamp?: string } = {},
		timeout = 30000
	) {
		const { width = 20, height = 0, quality = 40, timestamp = "00:00:01.000" } = options;
